...
class NewsController extends Controller
{
    ...
    public function search(Request $request){
        ...
        $model_News->offset($offset);
        // $rows = $model_News->with("author")->with('translation')->with("category")->paginate(2);
        $rows = $model_News->with("author")->with('translation')->with("category")->limit(5)->get();
        $total = $rows->count();
    ...
    public function viewUpdate(News $news)
    {        
        $news->views += 1;
        $news->update();
