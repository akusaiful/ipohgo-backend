...
@section('content')
    <div class="container-fluid">
        <div class="d-flex justify-content-between mb20">
            <h1 class="title-bar">{{ __('All Notification') }}</h1>
            <div class="title-actions">
                <a href="{{ route('notification.admin.create') }}"
                    class="btn btn-primary">{{ __('Add new notification') }}</a>
            </div>
        </div>
        ...
        <div class="filter-div d-flex justify-content-between ">
            <div class="col-left">
                @if (!empty($rows))
                    <form method="post" action="{{ route('notification.admin.bulkEdit') }}"
            ...
            <div class="col-left">
                <form method="get" action="{{ route('notification.admin.index') }} "
                    ...
                    <button class="btn-info btn btn-icon btn_search"
                        type="submit">{{ __('Search Notification') }}</button>
                </form>
            </div>
        </div>
                                ...
                                <table class="table table-hover">
                                    <thead>
                                        <tr>
                                            ...
                                            <th class="title" style="text-align:center"> {{ __('Send') }}</th>
                                            ...
                                            @foreach ($rows as $row)
                                                <tr>
                                                    ...
                                                    <td class="title" style="text-align: center">
                                                        @if ($row->send)
                                                            <img src="{{ asset('images/correct.png') }}" width="20px" alt="">
                                                        @else
                                                        <img src="{{ asset('images/snooze.png') }}" width="25px" alt="">
                                                        @endif
                                                    </td>
                                                    ...
                                                    <td>
                                                        <a href="{{ route('notification.admin.edit', ['id' => $row->id]) }}"
