...
class Tour extends Bookable
{
    ...
    {
        return $this->hasMany("Modules\User\Models\UserWishList", "object_id", 'id')->where([
            'object_model' => 'tour'
        ]);
    }
    ...
    public function search($request)
    {
        ...
        if (!empty($request['title'])) {
            $title = $request['title'];
            $model_Tour->where('bravo_tours.title', 'like', "%$title%");
        }
    ...
    public function dataForMobile($forSingle = false)
    {
        ...
        // $data = parent::dataForApi($forSingle);
        $data['id'] = $this->id;
        $data['category_id'] = $this->category_tour->id;
        $data['category'] = $this->category_tour->name;
        $data['state'] = Location::selectRaw("id,name")->find($this->location_id) ?? null;
        $data['place name'] = $translation->title;
        $data['location'] = $this->address;
        $data['latitude'] = $this->map_lat;
        $data['longitude'] = $this->map_lng;
        $data['description'] = $translation->short_desc;
        $data['content'] = $translation->content;
        $data['paid_parking']     = $this->paid_parking ? true : false;
        $data['free_parking']     = $this->free_parking ? true : false;
        $data['faqs'] = $this->faqs;
        $data['include'] = $this->include;
        $data['exclude'] = $this->exclude;
        $data["itinerary"] = null;
        ...
        if ($this->image_id) {
            $data["featured_image"] = get_file_url($this->image_id, 'full');
        } else {
            $data["featured_image"] = get_file_url(261, 'full');
        }
        ...
        if (!empty($this->gallery)) {
            ...
            foreach ($galleries as $item) {
                if ($item) {
                    $data['sliders'][] = get_file_url($item, 'full');
                }
                $data["image-" . $i] = get_file_url($item, 'full');                
                $i++;
            }
        }
        // $data['image-1'] = 'https://images.unsplash.com/photo-1695653420644-ab3d6a039d53?ixlib=rb-4.0.3&ixid=M3wxMjA3fDF8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=4138&q=80';
        // $data['image-2'] = 'https://images.unsplash.com/photo-1695653420644-ab3d6a039d53?ixlib=rb-4.0.3&ixid=M3wxMjA3fDF8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=4138&q=80';
        // $data['image-3'] = 'https://images.unsplash.com/photo-1695653420644-ab3d6a039d53?ixlib=rb-4.0.3&ixid=M3wxMjA3fDF8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=4138&q=80';
        $data['loves'] = $this->wishlist_count ?: 0;
        // $data['comments count'] = 10;
        $data['date'] = Carbon::parse($this->updated_at)->format('d-m-Y G:i:s A');

        // fake timestamp to support firebase collection
        $data['timestamp'] = '1696230060' . rand();
        ...
        // $data['review_lists'] = $this->getReviewList();
        // $data['category'] = TourCategory::selectRaw("id,name")->find($this->category_id) ?? null;
        $reviews = $this->reviewClass::select(['id', 'title', 'content', 'rate_number', 'author_ip', 'status', 'created_at', 'vendor_id', 'author_id'])
            ->where('object_id', $this->id)
            ->where('object_model', 'tour')
            ->where("status", "approved")
            ->inRandomOrder()
            ...
            ->limit(5)
            ->get();
        $data['reviews'] = $reviews->map(function ($row) {
