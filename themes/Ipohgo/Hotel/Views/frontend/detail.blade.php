                                    ...
                                    <div class="px-2 pt-2">
                                        @if($row->map_lat && $row->map_lng)
                                            <a target="_blank" href="https://www.google.com/maps/place/{{$row->map_lat}},{{$row->map_lng}}/{{'@'.$row->map_lat}},{{$row->map_lng}},{{!empty($row->map_zoom) ? $row->map_zoom : 12}}z" class="d-block border rounded mb-4">
                                                <img class="img-fluid" src="{{  url("themes/ipohgo/images/map.jpg") }}" alt="{{__('Address-Description')}}">
                                            </a>
                                        @endif
                                    </div>
                                </div>
...
@push('js')
    ...
    <script type="text/javascript" src="{{ asset('themes/ipohgo/module/hotel/js/single-hotel.js?_ver='.config('app.asset_version')) }}"></script>
@endpush
